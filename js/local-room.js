const LocalRoom = (function () {
  ...
  function catalog() {
    return typeof FALLBACK_TITLES !== 'undefined' ? FALLBACK_TITLES : [];
  }
  ...
  function applyPlayback(room, cmd) {
    ...
    if (!pb.paused && pb.updatedAt) pb.t += (now - pb.updatedAt) / 1000;
  ...
  function getPlaybackNow(room) {
    const pb = { ...room.playback };
    if (!pb.paused && pb.updatedAt) pb.t += (Date.now() - pb.updatedAt) / 1000;
    return pb;
  }
